...
--
	...
	function m.files(prj)
		...
		tree.traverse(tr, {
			...
			onleaf = function(node, depth)
				if ends_with(node.relpath,".h") or ends_with(node.relpath,".hpp") then 
					p.w('set_source_files_properties ("%s" PROPERTIES HEADER_FILE_ONLY TRUE)',"../" .. node.relpath)
				end
			end,
		}, true)
	...
	local function outputof_table(tbl) 
		...
		for _,x in ipairs(tbl) do
			--print (x)
			if outputs_cache[x] == nil then
				local o = os.outputof(string.format('echo "%s "', x))
				--print (o)
				table.insert(t,o)
			else
				table.insert(t,outputs_cache[x])
			end
		end
		return t
	end
	...
	function m.compiler(prj,cfg)
		...
		local cxxflagstbl = outputof_table(table.join(toolset.getcxxflags(cfg), forceincludes, cfg.buildoptions))
		--local cxxflags = table.concat(cxxflagstbl, " ")
		local cflagstbl  = outputof_table(table.join(toolset.getcflags(cfg), forceincludes, cfg.buildoptions))
		--local cflags   = table.concat(cflagstbl, " ")
		local asmflags = ""
		local pch      = "" 

		--p.push("target_compile_options(%s PUBLIC ",prj.name)
	--#p.w(table.concat(cxxflagstbl," "))
		--p.pop(")")

		p.w('set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} %s")',table.concat(cflagstbl," "))

		p.w('set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS}  %s")', table.concat(cxxflagstbl," ")) 

		--p.w('set (CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} %s")', cxxflags)
		--p.w('set (CMAKE_C_FLAGS "${CMAKE_C_FLAGS} %s")', cflags)
		...
		for _, includedir in ipairs(cfg.includedirs) do
			p.w('"%s"',"../" .. project.getrelative(cfg.project, includedir))
	...
	function m.linker(prj,cfg)
		...
		local toolset = m.getcompiler(cfg) 
		local flagstbl = outputof_table(table.join(toolset.getldflags(cfg), cfg.linkoptions, toolset.getlinks(cfg)))
		--_x(3, '<Linker Required="yes" Options="%s">', table.concat(flags, ";"))
		p.w('target_link_libraries (%s PUBLIC %s)', prj.name, table.concat(flagstbl," ")) 
		
		p.w('target_link_directories (%s PUBLIC %s)', prj.name, table.concat(cfg.libdirs," ")) 
	...
	function m.settings(prj)
		...
		p.w("SET(CMAKE_BUILD_TYPE Debug")
		p.w("    CACHE STRING \"Choose the type of build : None Debug Release RelWithDebInfo MinSizeRel Coverage.\"")
		p.w("    FORCE)")
		...
		for cfg in project.eachconfig(prj) do
			...
			if cfgname:sub(1,3) ~= "x32" then
				cfgname = cfgname:sub(5)
				...
				p.w("endif (CMAKE_BUILD_TYPE STREQUAL %s)", cfgname)
			end
			--_p(2, '</Configuration>')
		end
